...
<script type="text/javascript">
	...
	function dealUserBuyerReview(){
		 if(confirm(" 후기 작성 하시려면 예를 누르시고\n후기 작성 하지 않으시려면 아니오를 눌러주세요")){
			 location.href="/shop/user/deal/dealUserBuyerComplete.do?d_idx=${dealUserVO.d_idx}"; // 구매 후기 주소 바꾸고 주석 제거해 주세요.
		 }
		 else{
		 alert('아니오를 누르셨습니다');
		 }
...
<div class="container text-center">
    ...
    <table class="table">
		...
		<tr>
			<th>판매자[신용]</th>
			<td>${dealUserVO.s_nickname}[${dealUserVO.seller_score}]
			</td>
		</tr>
		<tr>
			<th>상품명</th>
			<td>${dealUserVO.p_name }
			</td>
		</tr>
		<tr>
			<th>상품 수량</th>
		...
		<tr>
			<th>가격</th>
			<td><fmt:formatNumber type="number" maxFractionDigits="3" value="${dealUserVO.p_price * dealUserVO.d_q}"/>원
			</td>
		</tr>
		<tr>
			<th>판매자 연락처</th>
			<td>${dealUserVO.s_phone }
			</td>
		</tr>
		<tr>
			<th>판매자 계좌</th>
			<td>${dealUserVO.s_account}
			</td>
		</tr>
		<tr>
			<th>내용</th>
	...
	<div id="buton">
	<c:choose>
		<c:when test="${dealUserVO.d_ing eq '1' || dealUserVO.d_ing eq '2' || dealUserVO.d_ing eq '3'}">
    		<button type="button" class="btn btn-danger"  onclick="dealUserBuyerCancel()">거래 취소</button>
		</c:when>
	</c:choose> 			

	<c:choose>
		<c:when test="${dealUserVO.d_ing == '3'}">
  	 	  	<button type="button" class="btn btn-success"  onclick="dealUserBuyerComplete()">거래 완료</button>
		</c:when>
	</c:choose> 			

	<c:choose>
		<c:when test="${dealUserVO.d_ing == '4'}">
    		<button type="button" class="btn btn-info"  onclick="dealUserBuyerReview()">후기 작성</button>
		</c:when>
	</c:choose>
	</div>
	</div>
</div>
   	<button type="button" class="btn btn-warning"  onclick="location.href='/shop/user/deal/dealUserBuyerList.do'"><i class="fa fa-bars">목록</i></button><br>
</div>
